...
BASE_PATH = "./VizDoomGym/scenarios"
...
class DoomEnv(gym.Env):
    def __init__(self, config, frame_skip=1, frame_stack=1, down_sample=(None, None), to_torch=True, normalize=True,
        ...
        # End episode when out of ammo for 4 steps in a row
        self.__add_game_variables(AMMO)
        self.same = 0
        # init game
        try:
    ...
    def step(self, action):
        ...
        # End episode when out of ammo for 4 steps in a row
        if self.game.get_state() and self.game.get_state().game_variables[2] == 0:
            self.same += 1
        else:
            self.same = 0
        done = int(self.game.is_episode_finished()) or (self.same == 4)
        self.__append_observation_buffers()
        observation = self.__get_observation_frames()
    ...
    def __parse_observation_space(self):
        ...
        channels = self.game.get_screen_channels()
        if channels == 1:
            channels = 0
